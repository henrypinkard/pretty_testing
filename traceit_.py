...
Auto-loaded into builtins — no import needed.
...
Options:
    max_depth=6      Max recursion depth to print
    show_returns=True   Show return values (└─>)
    max_len=None     Truncate long reprs (None = no limit)
...
Examples:
    @traceit_(max_depth=10, show_returns=False)
    def fibonacci(n): ...

    @traceit_(watch=[2, 3])  # Only show low, high
    def binary_search(arr, target, low, high): ...

    @traceit_(show_depth=True)  # See exact recursion depth
...
def _smart_truncate(obj, max_len=None):
...
def traceit_(_func=None, *, max_depth=6, show_returns=True, max_len=None, indent="\t",
             watch=None, show_depth=False, show_exc=True, limit=None, verbose=True):
    """
    Decorator to trace recursive function calls.

    Args:
        max_depth: Maximum recursion depth to print (default 6)
        show_returns: Whether to print return values (default True)
        max_len: Maximum length for argument/return value repr (default None)
    ...
    Examples:
        ...
        @traceit_(max_depth=5, show_returns=False)
        def fibonacci(n): ...

        @traceit_(watch=[2, 3])  # Only show low, high for binary search
        def binary_search(arr, target, low, high): ...

        @traceit_(show_depth=True)  # Show [0], [1], [2] prefix
        def deep_recursion(n): ...
    """
    def decorator(func):
        depth = [0]
        ...
        def wrapper(*args, **kwargs):
            ...
            if limit is not None and d >= limit:
                depth[0] -= 1
                raise RecursionError(f"traceit_ limit={limit} exceeded")
