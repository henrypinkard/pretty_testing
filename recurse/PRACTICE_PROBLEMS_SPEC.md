...
This directory uses the `traceit_` decorator (auto-injected into builtins, no import needed) for debugging recursive functions. The goal is to generate practice problems with intentional bugs so the user can practice:
...
4. **Suggest using `@traceit_` with appropriate options**

The user should:
1. Add `@traceit_` decorator
2. Run the test case
