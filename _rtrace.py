...
Options:
    ...
    max_len=250      Truncate long reprs
...
Examples:
    @_rtrace(max_depth=10, show_returns=False)
    def fibonacci(n): ...

    @_rtrace(watch=[2, 3])  # Only show low, high
    def binary_search(arr, target, low, high): ...

    @_rtrace(show_depth=True)  # See exact recursion depth
...
def _rtrace(_func=None, *, max_depth=6, show_returns=True, max_len=250, indent="\t",
    ...
    Args:
        ...
        max_len: Maximum length for argument/return value repr (default 250)
    ...
    Examples:
        ...
        @_rtrace(max_depth=5, show_returns=False)
        def fibonacci(n): ...

        @_rtrace(watch=[2, 3])  # Only show low, high for binary search
        def binary_search(arr, target, low, high): ...

        @_rtrace(show_depth=True)  # Show [0], [1], [2] prefix
        def deep_recursion(n): ...
    """
    def decorator(func):
        depth = [0]
        ...
        def wrapper(*args, **kwargs):
            ...
            if limit is not None and d >= limit:
                depth[0] -= 1
                raise RecursionError(f"_rtrace limit={limit} exceeded")
